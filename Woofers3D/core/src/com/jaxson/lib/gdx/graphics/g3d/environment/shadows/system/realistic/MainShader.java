...
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.system.BaseShadowSystem.LightProperties;
import com.jaxson.lib.gdx.io.GdxFile;

/**
 * This shader is used by the realistic shadow system. This shader supports
...
public class MainShader extends DefaultShader
{
	...
	protected final int u_dirShadows0uvTransform
			= register(new Uniform("u_dirShadows[0].uvTransform"));
	protected final int u_dirShadows1uvTransform
			= register(new Uniform("u_dirShadows[1].uvTransform"));
	...
	protected int u_dirShadowMapProjViewTrans0
			= register(new Uniform("u_dirShadowMapProjViewTrans[0]"));
	protected int u_dirShadowMapProjViewTrans1
			= register(new Uniform("u_dirShadowMapProjViewTrans[1]"));
	...
	protected int u_dirShadowMapUVTransform0
			= register(new Uniform("u_dirShadowMapUVTransform[0]"));
	protected int u_dirShadowMapUVTransform1
			= register(new Uniform("u_dirShadowMapUVTransform[1]"));
	...
	protected final int u_spotShadows0uvTransform
			= register(new Uniform("u_spotShadows[0].uvTransform"));
	protected final int u_spotShadows1uvTransform
			= register(new Uniform("u_spotShadows[1].uvTransform"));
	...
	protected int u_spotShadowMapProjViewTrans0
			= register(new Uniform("u_spotShadowMapProjViewTrans[0]"));
	protected int u_spotShadowMapProjViewTrans1
			= register(new Uniform("u_spotShadowMapProjViewTrans[1]"));
	...
	protected int u_spotShadowMapUVTransform0
			= register(new Uniform("u_spotShadowMapUVTransform[0]"));
	protected int u_spotShadowMapUVTransform1
			= register(new Uniform("u_spotShadowMapUVTransform[1]"));
	...
	{
		final DirectionalLightsAttribute dla = attributes.get(
				DirectionalLightsAttribute.class,
		...
		if (dirLightsLoc >= 0)
		{
			for (int i = 0; i < directionalLights.length; i ++)
			{
				...
				// Shadow
				ObjectMap<DirectionalLight, LightProperties> dirCameras
						= shadowSystem.getDirectionalCameras();
				...
				if (shadowSystem.hasLight(dl))
				{
					...
					{
						program.setUniformf(
								idx + dirShadowsUvTransformOffset, tr.getU(),
								tr.getV(), tr.getU2() - tr.getU(), tr.getV2()
										- tr.getV());

						// ProjViewTrans
						idx = dirShadowMapProjViewTransLoc
								+ i * dirShadowMapProjViewTransSize;
						program.setUniformMatrix(
								idx, dirCameras.get(dl).camera.combined);
					}
				}
	...
	{
		final SpotLightsAttribute sla = attributes
				.get(SpotLightsAttribute.class, SpotLightsAttribute.Type);
		...
		if (spotLightsLoc >= 0)
		{
			for (int i = 0; i < spotLights.length; i ++)
			{
				...
				// Shadow
				ObjectMap<SpotLight, LightProperties> spotCameras
						= shadowSystem.getSpotCameras();
				...
				if (shadowSystem.hasLight(sl))
				{
					...
					{
						program.setUniformf(
								idx + spotShadowsUvTransformOffset, tr.getU(),
								tr.getV(), tr.getU2() - tr.getU(),
								tr.getV2() - tr.getV());

						// ProjViewTrans
						idx = spotShadowMapProjViewTransLoc
								+ i * spotShadowMapProjViewTransSize;
						program.setUniformMatrix(
								idx, spotCameras.get(sl).camera.combined);
					}
				}
	...
	public void init()
	{
		...
		dirShadowsUvTransformOffset
				= loc(u_dirShadows0uvTransform) - dirShadowsLoc;
		...
		dirShadowMapUVTransformSize
				= loc(u_dirShadowMapUVTransform1) - dirShadowMapUVTransformLoc;

		// Spot Shadow
		spotShadowsLoc = loc(u_spotShadows0uvTransform);
		spotShadowsUvTransformOffset
				= loc(u_spotShadows0uvTransform) - spotShadowsLoc;
		...
		spotShadowMapProjViewTransSize = loc(u_spotShadowMapProjViewTrans1)
				- spotShadowMapProjViewTransLoc;

		spotShadowMapUVTransformLoc = loc(u_spotShadowMapUVTransform0);
		spotShadowMapUVTransformSize = loc(u_spotShadowMapUVTransform1)
				- spotShadowMapUVTransformLoc;
	}
	...
	public static String getDefaultFragmentShader()
	{
		if (defaultFragmentShader == null)
			defaultFragmentShader = new GdxFile("shaders/realistic/main.fragment.glsl").readString();
	...
	public static String getDefaultVertexShader()
	{
		if (defaultVertexShader == null)
			defaultVertexShader = new GdxFile("shaders/realistic/main.vertex.glsl").readString();
