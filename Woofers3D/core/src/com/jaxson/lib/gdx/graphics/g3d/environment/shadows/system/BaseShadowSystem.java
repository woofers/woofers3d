...
package com.jaxson.lib.gdx.graphics.g3d.environment.shadows.system;
...
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.AABBNearFarAnalyzer;
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.BoundingSphereDirectionalAnalyzer;
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.DirectionalAnalyzer;
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.FixedShadowMapAllocator;
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.FrustumLightFilter;
...
import com.jaxson.lib.gdx.graphics.g3d.environment.shadows.utils.ShadowMapAllocator.ShadowMapRegion;
...
public abstract class BaseShadowSystem implements ShadowSystem, Disposable
{
	...
	{
		public ObjectMap<CubemapSide, LightProperties> properties
				= new ObjectMap<>(6);
	}
	...
	public BaseShadowSystem()
	{
		this(new AABBNearFarAnalyzer(),
				new FixedShadowMapAllocator(FixedShadowMapAllocator.QUALITY_MED,
	 ...
	 * @param directionalAnalyzer Analyze directional light to create
	 * orthographic camera
	 * @param lightFilter Filter light to render
	 */
	public BaseShadowSystem(NearFarAnalyzer nearFarAnalyzer,
	...
	public void addLight(PointLight point)
	{
		addLight(
				point,
				EnumSet.of(
						CubemapSide.PositiveX, CubemapSide.NegativeX,
						CubemapSide.PositiveY, CubemapSide.NegativeY,
	...
	public void addLight(SpotLight spot)
	{
		PerspectiveCamera camera
				= new PerspectiveCamera(spot.cutoffAngle, 0, 0);
		camera.position.set(spot.position);
		camera.direction.set(spot.direction);
		camera.near = 1;
		camera.far = 100;
		camera.up.set(
				camera.direction.y, camera.direction.z, camera.direction.x);

		spotCameras.put(spot, new LightProperties(camera));
	}

	@Override
	public <T extends RenderableProvider> void begin(Camera camera,
	...
	public void begin(int n)
	{
		if (n >= passShaderProviders.length)
			throw new GdxRuntimeException(
					"Pass " + n + " doesn't exist in " + getClass().getName());
	...
	{
		if (currentPass != n) throw new GdxRuntimeException(
				"Begin " + n + " must be called before end " + n);
		endPass(n);
	}
	...
	/**
	 * Initialize framebuffers and shader providers. You should call
	 * super.init() in subclass.
	 */
	@Override
	public void init()
	{
	...
	protected LightProperties nextDirectional()
	{
		...
		ObjectMap.Entry<DirectionalLight, LightProperties> e
				= dirCameraIterator.next();
		currentLight = e.key;
	...
	protected LightProperties nextPoint()
	{
		...
		{
			ObjectMap.Entry<PointLight, PointLightProperties> e
					= pointCameraIterator.next();
			currentLight = e.key;
			currentPointProperties = e.value;
		}

		if (currentPointProperties.properties
				.containsKey(Cubemap.CubemapSide.values()[currentPointSide]))
		{
			LightProperties lp = currentPointProperties.properties
					.get(Cubemap.CubemapSide.values()[currentPointSide]);
			currentLightProperties = lp;
			currentPointSide += 1;

			if (!lightFilter.filter(
					pointCameras.findKey(currentPointProperties, true),
					lp.camera, this.camera))
			{
				return nextPoint();
			}
	...
	protected LightProperties nextSpot()
	{
		...
		ObjectMap.Entry<SpotLight, LightProperties> e
				= spotCameraIterator.next();
		currentLight = e.key;
		currentLightProperties = e.value;
		LightProperties lp = e.value;

		if (!lightFilter
				.filter(spotCameras.findKey(lp, true), lp.camera, this.camera))
		{
			return nextSpot();
		}
	...
	public void update()
	{
		...
		{
			for (ObjectMap.Entry<
				CubemapSide, LightProperties> c: e.value.properties)
			{
				c.value.camera.position.set(e.key.position);
				nearFarAnalyzer
						.analyze(e.key, c.value.camera, renderableProviders);
			}
		}
	}
}
