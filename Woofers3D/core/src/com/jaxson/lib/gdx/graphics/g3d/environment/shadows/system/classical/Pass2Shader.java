...
package com.jaxson.lib.gdx.graphics.g3d.environment.shadows.system.classical;
...
public class Pass2Shader extends DefaultShader
{
	...
	{
		public final static Uniform shadowMapProjViewTrans
				= new Uniform("u_shadowMapProjViewTrans");
		public final static Uniform shadowTexture
				= new Uniform("u_shadowTexture");
		...
		public final static Uniform lightDirection
				= new Uniform("u_lightDirection");
		public final static Uniform lightIntensity
				= new Uniform("u_lightIntensity");
		public final static Uniform lightPosition
				= new Uniform("u_lightPosition");
		public final static Uniform lightCutoffAngle
				= new Uniform("u_lightCutoffAngle");
		public final static Uniform lightExponent
				= new Uniform("u_lightExponent");
	}
	...
	public static class Setters extends DefaultShader.Setters
	{
		...
		{
			@Override
			public void set(BaseShader shader, int inputID,
					Renderable renderable, Attributes combinedAttributes)
			{
				shader.set(
						inputID, shadowSystem
								.getCurrentLightProperties().camera.combined);
			}
		};
		...
		public final static Setter uvTransform = new GlobalSetter()
		{
					...
					Renderable renderable, Attributes combinedAttributes)
			{
				final TextureRegion tr
						= shadowSystem.getCurrentLightProperties().region;
				shader.set(
						inputID, tr.getU(), tr.getV(), tr.getU2() - tr.getU(),
						tr.getV2() - tr.getV());
			}
		};
		public final static Setter lightColor = new GlobalSetter()
		{
					...
					Renderable renderable, Attributes combinedAttributes)
			{
				...
				shader.set(
						inputID, l.color.r * intensity, l.color.g * intensity,
						l.color.b * intensity);
			}
		};
		public final static Setter lightDirection = new GlobalSetter()
		{
					...
					Renderable renderable, Attributes combinedAttributes)
			{
				...
				if (l instanceof PointLight)
				{
					shader.set(
							inputID, shadowSystem
									.getCurrentLightProperties().camera.direction);
				}
			}
		};
		...
		public final static Setter lightCutoffAngle = new GlobalSetter()
		{
			...
			{
				if (!(shadowSystem
						.getCurrentLight() instanceof DirectionalLight))
				{
					shader.set(
							inputID, ((PerspectiveCamera) shadowSystem
									.getCurrentLightProperties().camera).fieldOfView);
				}
	...
	protected BlendingAttribute blend
			= new BlendingAttribute(GL20.GL_ONE, GL20.GL_ONE);
	...
	public boolean canRender(Renderable renderable)
	{
		...
		boolean dir
				= shadowSystem.getCurrentLight() instanceof DirectionalLight;
			...
			final Config config)
	{
		...
		boolean dir = config.shadowSystem
				.getCurrentLight() instanceof DirectionalLight;
		if (dir)
	...
	public static String getDefaultFragmentShader()
	{
		if (defaultFragmentShader == null)
			defaultFragmentShader = new GdxFile("classical/pass2.fragment.glsl").readString();
	...
	public static String getDefaultVertexShader()
	{
		if (defaultVertexShader == null)
			defaultVertexShader = new GdxFile("shaders/classical/pass2.vertex.glsl").readString();
