...
public class BoundingSphereDirectionalAnalyzer implements DirectionalAnalyzer
{
	...
	public Camera analyze(DirectionalLight light, Camera out, Camera mainCamera)
	{
		...
		for (Vector3 planePoint: mainCamera.frustum.planePoints)
		{
			bb.ext(planePoint);
		}
		...
		if (d.z < d.x + d.y)
			out.up.set(
					-light.direction.y, light.direction.x, light.direction.z);
		else
			out.up.set(
					light.direction.x, -light.direction.z, light.direction.y);
