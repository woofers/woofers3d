...
public class ChaseCamera extends PerspectiveCamera
{
	...
	public void update(final float delta, final boolean updateFrustum)
	{
		...
		if (offsetBounds.isValid())
		{
			...
			if (temporary.x < offsetBounds.min.x) position.x = offsetBounds.min.x + direction.x;
			if (temporary.x > offsetBounds.max.x) position.x = offsetBounds.max.x + direction.x;
			if (temporary.y < offsetBounds.min.y) position.y = offsetBounds.min.y + direction.y;
			if (temporary.y > offsetBounds.max.y) position.y = offsetBounds.max.y + direction.y;
			if (temporary.z < offsetBounds.min.z) position.z = offsetBounds.min.z + direction.z;
			if (temporary.z > offsetBounds.max.z) position.z = offsetBounds.max.z + direction.z;
		}
